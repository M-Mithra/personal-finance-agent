...
The fourth slice (local LLM integration) is now designed and its feasibility experiments are complete: `docs/11_local_llm_integration.md` documents the experimental plan and the empirical results. Feasibility experiments were performed on Apple Silicon M2 (8 GB unified memory) with Ollama 0.33.3 using Apple Metal acceleration, comparing Qwen3 4B and Qwen3 1.7B. Qwen3 1.7B is the **provisional** first implementation target; Qwen3 4B remains a comparison baseline. The provider-neutral `LLMClient` remains the architectural boundary. The first concrete provider adapter (`llm/ollama.py`, `OllamaLLMClient`) is now implemented behind the provider-neutral interface, using stdlib HTTP only (no SDK dependency). The LLM-driven agent execution loop is the current implementation focus (Slice 7, DEC-023): `runtime/llm_loop.py` wires the provider-neutral `LLMClient` into the deterministic runtime loop as a bounded reasoning subroutine, preserving the deterministic runtime as the control boundary.
...
| LLM Provider Adapter | `llm/ollama.py` | First concrete `LLMClient` implementation: adapts the provider-neutral interface to Ollama's local HTTP API. stdlib HTTP only; no SDK dependency. |
| LLM-Driven Execution Loop | `runtime/llm_loop.py` | Bounded LLM-driven agent loop. Uses `LLMClient.complete()` as a reasoning subroutine inside the existing deterministic runtime control flow. Validates, executes (via existing `tools.execute_tool`), verifies (via existing `checks.verify_for`), and updates `AgentState`. Includes a deterministic grounding gate on final responses. The existing deterministic `run()` and lifecycle remain the default; the LLM loop is opt-in (`run_llm_loop`). |
...
## Slice 6 - Ollama-backed LLM provider adapter

**Status: implemented.** First concrete `LLMClient` implementation (`llm/ollama.py`, `OllamaLLMClient`) adapts the provider-neutral interface to Ollama's local HTTP API (`POST /api/chat`). Uses stdlib HTTP only (`urllib.request`, `json`, `socket`); no SDK dependency. Non-streaming initially (`stream: false`). Model, base URL, timeout, and generation options are configurable. Provider/transport failures are mapped to the existing provider-neutral error hierarchy. JSON format is request-dependent rather than unconditional. The adapter remains behind the provider-neutral `LLMClient`; the deterministic runtime remains the control boundary.

## Slice 7 - LLM-driven agent execution loop

**Status: proposed (DEC-023, provisional/experimental); implementation complete, awaiting review.** Adds a bounded LLM-driven execution path (`runtime/llm_loop.py`) that uses `LLMClient.complete()` as a reasoning subroutine within the existing deterministic runtime control flow. The loop validates model proposals through `validate_model_response` and a new `validate_llm_tool_call` (structural -> contract -> semantic; reversed `period_comparison` periods rejected at the semantic layer, not normalized), executes approved tools via the existing `tools.execute_tool` (model `YYYY-MM` period arguments and `(year, month)` pairs coerced to `Period` objects first, with Python booleans rejected because `True`/`False` are `int` subclasses), verifies results through the existing `checks.verify_for`, updates `AgentState`, and repeats until answered, clarified, failed, or bounded. Invalid proposals retry within the same step (inner retry loop, `retry_invalid=1`); genuine model errors terminate as `model_error`. Hybrid pending-action handling: the matching planned action (by tool name) is consumed after each model tool call to avoid duplicates; remaining unproposed planned actions are gap-filled deterministically through the same execute -> verify path with budget checks. A deterministic grounding gate cross-checks currency-anchored monetary figures in final-response drafts against verified observations (bare numbers are not claims; correct-number/wrong-explanation may still pass). Bounds: `max_model_steps=6`, `max_tool_calls=4`, `max_consecutive_repeats=2` (identical tool+args), `retry_invalid=1`. The existing deterministic `run()` and lifecycle are unchanged and remain the default; the LLM loop is opt-in (`run_llm_loop`). Deterministic suite: `tests/test_llm_loop.py` (35 tests, all passing with `FakeLLMClient`); full suite 210 tests passing. Live Ollama `qwen3:1.7b` re-verified 2026-09-11 on "Why did my spending increase in September?" (see `docs/11_local_llm_integration.md` Experiment 6): two grounded `response_complete` runs (2 model calls + 2 verified tools each, ~16s/~43s, no retries); an earlier session run additionally exercised the budget-bound path (`budget_tool_calls` after a reversed-period rejection + gap-fill, deterministic fallback response, all verified). Fully unit-testable with `FakeLLMClient`; live Qwen3 tests live separately under `tests/integration/` and are skipped unless Ollama is reachable.
