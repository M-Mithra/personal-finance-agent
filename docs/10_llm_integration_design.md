...
**Implementation status (2026-08-09):** The conceptual `LLMClient` is now concrete in `src/personal_finance_agent/llm/`. `client.py` defines the `LLMClient` ABC (`model_identifier`, `complete`, `reason`, `generate_response`) plus an independent `validate_model_response` checker; `types.py` defines the structured response types (`ToolCall`, `ModelResponse`, `LLMRequest`, `ModelMetadata`); `tool_definitions.py`, `errors.py`, and `fake.py` define tool metadata, the error vocabulary, and the deterministic `FakeLLMClient`. The first concrete provider adapter, `OllamaLLMClient` (`ollama.py`), adapts the interface to Ollama's local HTTP API using stdlib only (no SDK). `run()` and `Agent` accept an injectable `llm_client` that records model identity on `AgentState` without changing the deterministic lifecycle. The LLM-driven execution loop is implemented in `runtime/llm_loop.py` (DEC-023, provisional/experimental): it calls `LLMClient.complete()` as a reasoning subroutine, validates proposals (structural -> contract -> semantic; reversed `period_comparison` periods rejected, not normalized), coerces model `YYYY-MM` period arguments to `Period` objects, executes tools, verifies results, and updates `AgentState` within bounded iteration (max_model_steps=6, max_tool_calls=4, max_consecutive_repeats=2 on identical tool+args, retry_invalid=1 with same-step retry). Hybrid pending-action handling consumes the matching planned action by name and gap-fills the rest through execute -> verify with budget checks. The final-response grounding gate checks currency-anchored monetary figures against verified observations (correct-number/wrong-explanation may pass by design). The provider-neutral `LLMClient` boundary is preserved; the deterministic runtime remains the control boundary.
