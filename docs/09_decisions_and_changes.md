...
### DEC-023 - LLM-Driven Agent Execution Loop

- **Date:** 2026-08-09
- **Area:** Agent Runtime / LLM Integration
- **Status:** PROPOSED (provisional/experimental — implementation complete 2026-09-11, awaiting review)
- **Decision:** Implement a bounded LLM-driven execution loop (`runtime/llm_loop.py`) that uses the provider-neutral `LLMClient` as a decision/reasoning subroutine within the existing deterministic runtime control flow. The loop calls `LLMClient.complete(LLMRequest)` to obtain either a tool-call proposal or a final response, validates every proposal through `validate_model_response` and `validate_llm_tool_call`, executes approved tools via the existing `tools.execute_tool`, verifies results through the existing `checks.verify_for`, updates `AgentState`, and repeats until answered, clarified, failed, or bounded. A deterministic grounding gate cross-checks monetary figures in a final-response draft against verified observations before accepting it; failures fall back to the deterministic `response.py` renderer. The existing `run()` and deterministic lifecycle remain unchanged and default; the LLM loop is opt-in (`run_llm_loop(request, transactions, llm_client, config)`). Bounds are `max_model_steps=6`, `max_tool_calls=4`, `max_consecutive_repeats=2`, `retry_invalid=1`. The first use case is the spending-change explanation ("Why did my September spending increase?").
- **Context / Problem:** The provider-neutral `LLMClient` and the deterministic runtime both exist independently. They must be connected so the LLM can guide planning, tool selection, and interpretation, while the deterministic runtime retains control of execution, validation, verification, state, and termination.
- **Alternatives Considered:** Integrating the model at the understanding layer only (keeps runtime deterministic but limits benefit); integrating at response generation only (limits reasoning); letting the LLM bypass verification (rejected — violates DEC-004 and DEC-013); adding an agent framework such as LangGraph (rejected — violates DEC-019 and minimalism principles).
- **Rationale:** The existing architecture — `AgentState`, trajectory, `tools.execute_tool`, `checks.verify_for`, `validate_model_response`, injectable `LLMClient`, `FakeLLMClient` — already provides every primitive the loop needs. A single focused module `runtime/llm_loop.py` can wire them together behind one opt-in entry point, leaving `run()` untouched and all 156 tests passing. Deterministic authority, independent verification, provider neutrality, and a provider-neutral interface are all preserved.
- **Consequences:** The agent can execute an LLM-driven trajectory while deterministic tools remain authoritative for computation, verification is independent of the model, and the runtime remains the control boundary. The loop is fully unit-testable with `FakeLLMClient` (no Ollama required); live Qwen3 tests live separately under `tests/integration/` and are skipped unless Ollama is reachable. The `LLMClient` boundary remains available for alternative providers.
- **Related Documentation:** `docs/10_llm_integration_design.md` (sections 1-3, 5, 9-11, 13), `docs/11_local_llm_integration.md`, `docs/06_implementation.md` (Slice 7), `docs/07_testing_and_evaluation.md`

> **Implementation note (2026-09-11, provisional/experimental — awaiting review, DEC-023 stays PROPOSED):** implementation complete in `runtime/llm_loop.py`. Public API: `run_llm_loop`, `build_llm_request`, `summarise_observation_for_llm`, `validate_llm_tool_call` (structural -> contract -> semantic), `grounding_gate`, `LLMLoopConfig` (max_model_steps=6, max_tool_calls=4, max_consecutive_repeats=2 on identical tool+args, retry_invalid=1 with inner same-step retry loop), `LLMTerminationReason`, `GroundingResult`. Reversed `period_comparison` periods REJECTED at semantic validation (not normalized; order defines `absolute_difference = total_b - total_a`); model `YYYY-MM`/`(year, month)` args coerced to `Period` before execution, with booleans rejected (Python `True`/`False` are `int` subclasses and must not be treated as `1`/`0`). Hybrid pending-actions: matching planned action consumed by tool name after each model call (no duplicates); remainder gap-filled via execute -> observe -> verify with budget checks (no validation bypass — the plan is deterministic — no unverified observations, failed gap-fill recorded as error observation). Grounding is currency-anchored figure matching only (bare years/counts ignored); correct-number/wrong-explanation may pass by design. `LLMRequest` frozen: retries rebuild the request; `FakeLLMClient`-raised malformed output is retryable, other `LLMError`s -> `model_error` (`ResponseSequenceExhaustedError` mid-retry -> `max_retries_invalid`). Evidence: `tests/test_llm_loop.py` 35/35 passing (FakeLLMClient), full suite 210/210 passing; live Ollama `qwen3:1.7b` "Why did my spending increase in September?" re-verified 2026-09-11 with two grounded `response_complete` runs (2 model calls + 2 verified tools each, ~16s/~43s, no retries, no invented figures; run A via `noteworthy_transactions`+`period_comparison`, run B via `spending_summary`+`period_comparison`); an earlier session run additionally exercised the budget-bound path (`budget_tool_calls` after one reversed-period rejection + gap-fill, deterministic fallback). See `docs/11_local_llm_integration.md` Experiment 6. Known limitations: repeat key per-(tool, coerced-args) with cross-arg reset; name-only gap-fill matching (accepted MVP tradeoff); live-model runs non-deterministic (budgets + fallback bound it).

### DEC-024 - Finalized Evaluation Specification (Provisional)

- **Date:** 2026-09-11
- **Area:** Testing / Evaluation
- **Status:** PROPOSED (provisional — specification finalized, runner not yet built, no live evaluation performed)
- **Decision:** Finalize the evaluation specification into a machine-readable case set under `eval/`: `ground_truth.json` (deterministic ground truth), `datasets.json` (Dataset A baseline; Dataset B planned/deferred), `cases_core.json` (primary benchmark EVAL-001..020), `cases_robust.json` (robustness/edge EVAL-021..028), and a consolidated `cases.json` (28 cases). Adopt a two-suite structure — a primary core benchmark (20 cases) evaluated separately from a robustness/edge suite (8 cases) — so error-path stress tests do not distort the primary research score. Define a preliminary 10-dimension scoring rubric, a valid-trajectory definition (for multi-step cases, required fact-establishing coverage plus optional supporting coverage, not one exact sequence), automated vs. human evaluation boundaries, and an expected report structure (core score, robustness score, overall score, category-level scores). Keep DEC-023 PROPOSED/PROVISIONAL/EXPERIMENTAL.
- **Context / Problem:** Earlier evaluation design (docs/07 sections 17-24) was purpose-setting and left the dataset, case set, metrics, and hierarchy TBD. A concrete, implementation-grounded specification was needed to unambiguously define what an evaluation runner executes before any runner is built or live model evaluation is run. A prior draft had described "20 core cases" plus "8 additional edge cases" without making the two suites explicit (the "20 vs 28" inconsistency).
- **Alternatives Considered:** Using a single undifferentiated 28-case set (rejected — mixes the primary benchmark with error-path tests, making the headline score ambiguous); deleting cases to force exactly 20 (rejected — useful robustness cases would be lost); selecting an external evaluation framework (rejected — no current dependency or infrastructure need).
- **Rationale:** Splitting into a primary core benchmark and a separate robustness suite keeps the headline research metric clean while preserving error-handling coverage. Ground truth is generated from the deterministic analytics module (not an LLM), so numerical facts are reproducible. Machine-readable JSON artifacts can feed a future runner without an external framework or dependency.
- **Consequences:** The next task can build a runner that executes EVAL-001..028 to produce core, robustness, overall, and category-level scores. Grounding and ambiguity limitations are documented as known limitations (the MVP monetary-figure gate does not verify explanation quality; keyword-based understanding does not map "food" to "Dining"). Dataset B, adversarial cases, and semantic-claim grounding remain future extensions excluded from the initial runner's required scope.
- **Related Documentation:** `docs/07_testing_and_evaluation.md` (section 34 and 17-24), `eval/ground_truth.json`, `eval/datasets.json`, `eval/cases_core.json`, `eval/cases_robust.json`, `eval/cases.json`
...
| LLM-Driven Execution Loop | Decision recorded (DEC-023, provisional/experimental); implementation complete 2026-09-11, awaiting review: `tests/test_llm_loop.py` 35/35 passing, full suite 210/210, live `qwen3:1.7b` verified (see Experiment 6 in `docs/11_local_llm_integration.md`) |
...
| 2026-08-09 | LLM-driven agent execution loop designed and approved (DEC-023): `runtime/llm_loop.py` wires `LLMClient.complete()` → `validate_model_response` + `validate_llm_tool_call` → existing `tools.execute_tool` → existing `checks.verify_for` → `AgentState.update`; bounded loop (6 steps / 4 tool calls / 2 repeats / 1 retry) with a deterministic grounding gate on final responses. Uses `FakeLLMClient` for unit tests; live Qwen3 test lives under `tests/integration/`. | Connect the provider-neutral LLM abstraction to the deterministic runtime as a reasoning subroutine while keeping the runtime the control boundary. | DEC-023 |
| 2026-09-11 | LLM-driven loop implementation completed and verified (DEC-023, provisional/experimental, awaiting review): `tests/test_llm_loop.py` 35/35 passing (FakeLLMClient), full suite 210/210 passing; live Ollama `qwen3:1.7b` re-verified on spending-change prompt (two grounded `response_complete` runs; earlier session run additionally exercised the budget-bound fallback path; see Experiment 6 in `docs/11_local_llm_integration.md`). Reversed periods rejected (not normalized); hybrid gap-fill kept with consume-by-name dedup + budget checks; currency-anchored grounding gate. | Record implementation evidence without changing DEC-023 status. | DEC-023 |
| 2026-09-11 | Finalized evaluation specification (DEC-024, provisional): machine-readable case set under `eval/` (ground_truth.json, datasets.json, cases_core.json, cases_robust.json, cases.json) with a primary core benchmark (EVAL-001..020), a separate robustness/edge suite (EVAL-021..028), a 10-dimension scoring rubric, valid-trajectory definition, automated vs. human evaluation boundaries, and expected report structure. Ground truth generated from deterministic analytics. No runner built; no live evaluation performed. DEC-023 stays PROPOSED. | Define unambiguously what an evaluation runner will execute before building it. | DEC-024 |
| 2026-09-12 | Evaluation CLI wired for LLM mode: `eval/__main__.py` now accepts `--mode llm`, `--model`, `--base-url`, `--timeout`; constructs `OllamaLLMClient` with Qwen3 1.7B default; injects client and `LLMLoopConfig` into `EvaluationRunner`; preserves deterministic mode unchanged. Tests verify deterministic path, LLM client injection, model identifier in report metadata, and no live Ollama calls in unit tests (using `FakeLLMClient`). | Enable first real LLM evaluation run against existing OllamaLLMClient without modifying production code. | DEC-024 |
| 2026-09-12 | Understanding layer fixes applied to `runtime/understanding.py`: (1) Reordered `_INTENT_PATTERNS` to preserve relative priority (SPENDING_CHANGE_EXPLANATION moved to end to avoid hijacking merchant queries); (2) Added `CATEGORY_SYNONYMS = {"food": "dining"}` with auto-mapping in `understand()`; (3) Narrowed "how much" patterns to avoid false positives on "spending increase"/"spend income"; (4) Added `UNSUPPORTED_KEYWORDS = ("tax", "investment", "credit", "loan")` pre-check. | Fix 5 of 8 reproducible LLM evaluation FAIL cases (EVAL-006, 011, 013, 016, 017, 018). | DEC-024 |
| 2026-09-12 | Intent preservation fix in `runtime/understanding.py`: when `understand()` recognizes an intent but finds no period, it now preserves the classified intent (e.g., CATEGORY_ANALYSIS) instead of overwriting it to AMBIGUOUS. Only `intent_status` is set to AMBIGUOUS. | Fix EVAL-011 actual_intent mismatch (was "ambiguous", expected "category_analysis"). | DEC-024 |
| 2026-09-12 | Deterministic intent enforced as constraint in LLM system prompt: `build_llm_request` in `runtime/llm_loop.py` changed from "Use this as guidance, not as a constraint to override" to "You must follow this intent and must not override it." | Fix EVAL-004 (LLM overrode MERCHANT_ANALYSIS → NOTEWORTHY_TRANSACTIONS). | DEC-024 |
| 2026-09-12 | Evaluation case definitions updated: EVAL-009 expected_intent changed from "ambiguous" to "spending_summary"; EVAL-010 from "ambiguous" to "spending_change_explanation"; EVAL-011 fact_establishing_tools and expected_deterministic_facts cleared (agent returns AMBIGUOUS without calling tools). Cases now reflect corrected intent recognition behavior. | Align case definitions with deterministic understanding behavior after synonym and intent-preservation fixes. | DEC-024 |
